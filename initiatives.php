...
// Fetch all initiatives images
$stmt = $pdo->prepare("SELECT * FROM gallery WHERE category = ? ORDER BY uploaded_at DESC");
$stmt->execute(['initiatives']);
$allInitiatives = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Group images by theme
$themes = [
  'literacy-numeracy' => 'Literacy & Numeracy Advancement',
  'sports-wellness' => 'Sports Development & Wellness',
  'environment-sustainability' => 'Environment & Sustainability Advocacy',
  'career-community' => 'Career Programming & Community Immersion',
  'technology-learning' => 'Technology Integration & Asynchronous Learning',
  'safe-resilient' => 'Safe & Resilient Learning Spaces',
  'initiativeposter' => 'Initiative Posters',
];
$themeImages = [];
...
$otherInitiatives = [];
foreach ($allInitiatives as $img) {
  $theme = $img['theme'] ?? '';
  if (isset($themeImages[$theme])) {
    $themeImages[$theme][] = $img;
  } else {
    $otherInitiatives[] = $img;
  }
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <title>Key Initiatives - SDOIN</title>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" />
  <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css" />
  <link rel="stylesheet" href="style.css" />
  <style>
    /* Copy gallery.php styles for grid, items, overlays, modal, etc. */
    .gallery-grid {
      display: grid;
...
<body>
    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="mainNav">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          <li class="nav-item"><a class="nav-link active" href="initiatives.php"><i class="fas fa-lightbulb me-1"></i>Key Initiatives</a></li>
  ...
  <section id="initiatives">
    <h2>Technologically Fortified Key Initiatives</h2>
  </section>
  <?php foreach ($themes as $key => $label): ?>
    <?php if (count($themeImages[$key]) > 0): ?>
      <section id="<?= htmlspecialchars($key) ?>">
        <h3><?= htmlspecialchars($label) ?></h3>
        <div class="gallery-grid">
          <?php foreach ($themeImages[$key] as $img): ?>
            <div class="gallery-item" data-bs-toggle="modal" data-bs-target="#imageModal"
              data-image="<?= htmlspecialchars(urlencode($img['filename'])) ?>"
              ...
              data-date="<?= date('F j, Y', strtotime($img['uploaded_at'])) ?>">
              <div class="gallery-image-container">
                <img src="uploads/gallery/<?= htmlspecialchars(urlencode($img['filename'])) ?>" alt="<?= htmlspecialchars($img['title']) ?>" class="gallery-image">
              ...
              <div class="gallery-content">
                ...
                <div class="gallery-description"><?= htmlspecialchars($img['description']) ?: 'No description available' ?></div>
                <div class="gallery-meta">
                  <span class="category-badge">
                    <i class="fas fa-lightbulb"></i>
                    <?= htmlspecialchars($label) ?>
                  </span>
                  <span><?= date('M j, Y', strtotime($img['uploaded_at'])) ?></span>
                </div>
              </div>
            </div>
          <?php endforeach; ?>
    ...
    <?php endif; ?>
  <?php endforeach; ?>
  <?php if (count($otherInitiatives) > 0): ?>
    <section id="other-initiatives">
      <h3>Other Initiatives</h3>
      <div class="gallery-grid">
        <?php foreach ($otherInitiatives as $img): ?>
          <div class="gallery-item" data-bs-toggle="modal" data-bs-target="#imageModal"
            data-image="<?= htmlspecialchars(urlencode($img['filename'])) ?>"
            data-title="<?= htmlspecialchars($img['title']) ?>"
            data-description="<?= htmlspecialchars($img['description']) ?>"
            data-category="Other Initiatives"
            data-date="<?= date('F j, Y', strtotime($img['uploaded_at'])) ?>">
            <div class="gallery-image-container">
              <img src="uploads/gallery/<?= htmlspecialchars(urlencode($img['filename'])) ?>" alt="<?= htmlspecialchars($img['title']) ?>" class="gallery-image">
              <div class="image-overlay">
